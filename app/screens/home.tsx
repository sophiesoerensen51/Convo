...
const HomeScreen = ({ navigation }) => {
  ...
  // Opdater brugerens info (fx displayName) når man vender tilbage til denne skærm
  useFocusEffect(
    useCallback(() => {
      const reloadUser = async () => {
        const currentUser = auth().currentUser;
        if (currentUser) {
          await currentUser.reload(); // opdater auth brugeren
          setUser(auth().currentUser); // sæt opdateret bruger i state
        }
      };
      reloadUser();
    }, [])
      ...
      {user && (
        <View style={styles.welcomeContainer}>
          <Text style={styles.welcomeText}>
            Velkommen, {user.displayName || user.email}
          </Text>
        </View>
      )}
